...
-- The APK assigns the transaction code before contacting the cloud so retries
-- after an ambiguous network failure resolve to the same order.
...
begin
  ...
  if v_code !~ '^P[A-HJ-NP-Z2-9]{8}$' then
    raise exception 'Invalid POS tracking code';
  end if;
  ...
  set tracking_code = v_code,
      ...
      express_fee = case
        when coalesce(p_express, false) then round(coalesce(v_order.subtotal, 0) * 0.50, 2)
        else 0
      end
  where id = v_order.id
