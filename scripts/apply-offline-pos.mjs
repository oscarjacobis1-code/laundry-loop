...
const offlineAuthEffect = `  useEffect(() => {
...
const syncBlock = `  const syncQueuedOfflineOrders = useCallback(async () => {
    ...
    if (synced > 0) {
      ...
      setMessage(`${synced} offline order${synced === 1 ? "" : "s"} synced to Laundry Loop cloud records.`);
    }
...
const helper = `  function saveCurrentPosOffline(items: Array<{ service: Service; qty: number }>) {
    ...
    const localOrder: Order = {
      ...
      items: orderItems,
      weight_summary: totalWeight > 0 ? `${totalWeight} lbs` : `${orderItems.length} service(s)`,
