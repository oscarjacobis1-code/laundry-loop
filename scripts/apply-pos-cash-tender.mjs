...
replaceOnce(
  ...
  '{pos.paymentMethod === "MMG" && <label>MMG transaction reference<input value={pos.paymentReference} onChange={(e) => setPos({ ...pos, paymentReference: e.target.value })} placeholder="Required for verification" maxLength={120} required /></label>}\n{pos.paymentMethod === "Cash" && <label className="ll-pos-original-tender">Cash received<input type="number" min="0" step="1" inputMode="numeric" value={pos.cashReceived} onChange={(e) => setPos({ ...pos, cashReceived: e.target.value })} placeholder={String(Math.ceil(posTotal))} required /></label>}\n{profile.role !== "staff" && <fieldset className="discount-box">',
  'cash tender UI',
);
